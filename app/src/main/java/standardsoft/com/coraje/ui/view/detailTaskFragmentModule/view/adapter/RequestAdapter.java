package standardsoft.com.coraje.ui.view.detailTaskFragmentModule.view.adapter;
...
public class RequestAdapter extends RecyclerView.Adapter<RequestAdapter.ViewHolder>{
    ...
    private List<PromptTask> mItems;

    // Instancia de escucha
    private OnItemClickListener mOnItemClickListener;

    public RequestAdapter(Context context, List<PromptTask> items) {
        this.mContext = context;
        this.mItems = items;
    }

    public interface OnItemClickListener{
        void onItemClick(PromptTask clickedPromptTask);
    }
    ...
    public ViewHolder onCreateViewHolder(@NonNull ViewGroup viewGroup, int i) {
        ...
        View view = layoutInflater.inflate(R.layout.item_prompt, viewGroup, false);
        return new ViewHolder(view);
    }

    @Override
    public void onBindViewHolder(ViewHolder holder, int position) {
        PromptTask promptTask = mItems.get(position);

        String task     = promptTask.getTask().trim();
        String module   = promptTask.getModule().getDescription();
        String status   = promptTask.getStatus().getDescription();
        String priority = promptTask.getPriority().getDescription();
        long date       = promptTask.getDate();
        int time        = promptTask.getEstimation();
